...
if (!defined("DRIVER")) {
	...
	*/
	function connect() {
		...
		require realpath(dirname(__FILE__) . '/../../../../config/settings.inc.php');
		if(!isset($_SESSION['prestashopAdmin']))
		{
			$tokenFile = realpath(dirname(__FILE__) . '/../../../../modules/ninjatools/adminerPermission.php');
			try
			{
			...
			//$now = time();
			//if(($now - filectime($tokenFile)) >= 60)
			//{
			//	echo 'Session timeout.';
			//	exit;
			//}
			//$permission comes from adminerPermission.php file.
			if($permission != 'accessGranted')
			{
				exit;
			}
			...
			$_SESSION['prestashopAdmin'] = $permission;
			
			
		}
		if ($connection->connect(_DB_SERVER_, _DB_USER_, _DB_PASSWD_)) {
