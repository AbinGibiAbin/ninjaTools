...
@set_time_limit(0);
error_reporting(0);
//check if called from within the module.
session_start();
$now = time();
if(isset($_GET['logout']))
{
	unset($_SESSION['prestashopPermission']);
	echo 'Logged out successfully.';
	exit;
}
if(!isset($_SESSION['prestashopPermission']))
{
	$file = dirname(__FILE__) . '/SSHAdminerPermission.php';
	if(!is_file($file))
	{
		echo 'No permission.';
		exit;
	}
	//die if the permission file is older than a minute.
	if(($now - filectime($file)) >= 60)
	{
		echo 'Session timeout.';
		exit;
	}
	include($file);
	//$permission comes from adminerPermission.php file.
	$_SESSION['prestashopPermission'] = $permission;
	$_SESSION['prestashopLastLogin'] = $now;
	unlink($file);
	
}
//die if no activity since last 15 minutes.
if(($now - $_SESSION['prestashopLastLogin']) >= 900)
{
	echo 'Session timeout.';
	exit;
}
...
<table id="main"><tr><td><?php if($s_auth){ ?>
	...
	<td><div class="headinfo"><a style="color:#fff;font-weight:bold;" href="?logout=1">Logout</a></a><?php echo $s_info; ?></div></td></tr></table>
	</div>
